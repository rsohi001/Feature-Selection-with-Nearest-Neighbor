...
def leave_one_out_cross_val_forwards(data, current_set, feature_to_be_added):
    ...
    temp_set = cp.deepcopy(current_set)
    temp_set.append(feature_to_be_added)
...
def leave_one_out_cross_val_backwards(data, current_set, feature_to_be_removed):
    temp_data = cp.deepcopy(data)
    temp_set = cp.deepcopy(current_set)
    temp_set.remove(feature_to_be_removed)
    length = len(temp_data[0])
    for i in range(1, length):
        if i not in temp_set:
            for k in range(len(temp_data)):
...
def feature_search_demo_backwards(data):
    ...
    for i in range(1, length):
        ...
        if(max_accuracy < best_accuracy):
            print("(Warning, accuracy has decreased! Continuing search in case of local maxima)")
        print("On level ", i, " feature ", feature_to_be_removed, "being removed, was best with a accuracy of", round(max_accuracy*100,1), "%")
    print("Best accuracy:", round(best_accuracy*100,1), "% with set ", ideal_set)

print("Welcome to Rahul Sohi's Feature Selection Algorithm")
